...
#route to the landing page when not authenticated / logged in
@app.route('/')
def landing():
...
# homepage when logged in, compilation of boards
@app.route('/boards')
@login_required
def boards():
    try:
...
# to-do list in boards / calculation of progress bar
...
def edit_card(card_id):
    ...
    if card.list.board.user_id != current_user.id:
        flash("You don't have access to this card.")
        return redirect(url_for('boards'))
    
    if request.method == 'POST':
        card.title = request.form.get('title')
        card.description = request.form.get('description', '')
        card.due_date = request.form.get('due_date')
        card.completed = 'completed' in request.form
        # Logic to set reminder flag
        if card.due_date:
            try:
                due_date_obj = datetime.strptime(card.due_date, '%Y-%m-%d').date()
...
# moving cards through boards / updating progress
